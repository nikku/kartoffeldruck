...
Kartoffeldruck.run = async function(options) {
  ...
  try {
    ...
    logger.error(colors.red('failed to load ' + runnerFile + ' runner: \n'), err);

    throw new Error('failed to load ' + runnerFile + ' runner in ' + cwd);
  }
  ...
  try {
    ...
    logger.error(colors.red('failed to run ' + runnerFile + ' runner: \n'), err);

    throw new Error('failed to run ' + runnerFile + ' runner in ' + cwd);
  }
