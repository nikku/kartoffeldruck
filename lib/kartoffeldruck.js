...
Kartoffeldruck.prototype.generate = async function(options) {
  ...
  var rendered = await this.generator.generate(pageOptions);
...
Kartoffeldruck.run = async function(options) {
  ...
  var time = Date.now();
  var druck = new Kartoffeldruck({
    cwd,
    logger
  });

  var runnerPath = path.resolve(cwd + '/' + runnerFile);
  ...
  try {
    ...
    throw new VError(err, 'failed to load <%s>', runnerPath);
  }

  try {
    await runner(druck);
  } catch (err) {
    throw new VError(err, 'failed to execute <%s>', runnerPath);
  }
