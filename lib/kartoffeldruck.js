...
Kartoffeldruck.run = function(directory, logger) {
  ...
  logger && logger.log('Generating site in ' + directory);

  var time = t();
  var blg = new Kartoffeldruck({ cwd: directory });

  var runner;

  try {
    runner = require(path.resolve(directory + '/' + runnerFile));
  } catch (e) {
    throw new Error('no ' + runnerFile + ' runner found in ' + directory);
  }
